...
function theComparator($a, $b)
{
  if (string_begins_with($a, 'The '))
    {
      $a = substr($a, 4);
    }
  if (string_begins_with($b, 'The '))
    {
      $b = substr($b, 4);
    }
		...
		try {
			...
			if($type==$ALBUM||$type==$ARTIST) {
				...
				if(strlen($dir)==0) {
					...
					/* SORT HERE */
      					//$res = sort_results_with_the_filter($res, strtolower($type));
					if ($type=="Artist") { usort($res[strtolower($type)], "theComparator"); }
					//$tmp = array_keys($res[strtolower($type)]);
					//$res[strtolower($type)][$tmp[0]] = $res[strtolower($type)][$tmp[5]];
					//$res[strtolower($type)][$tmp[0]] = "HEY BITCHES";
				}
				else {	
					$res = array();
					if($type==$ALBUM) {
				...
				//usort($res['directory'], "theComparator");
				if(isset($tmp['file'])) {
					$res['file'] = array();
